...
/**
 * Class inherent {@linkplain BitvectorDecoder} and decodes bits coded in
 * natural binary code to decimal values.
 * 
 * @author Branko
 *
 */
public class NaturalBinaryDecoder extends BitvectorDecoder {
	 ...
	 * 
	 * @param mins
	 *            desire lower limit for every variable
	 * @param maxs
	 *            desire upper limit for every variable
	 * @param bits
	 *            bits which will be decoded
	 * @param n
	 *            dimension
	 */
	...
	public double[] decode(BitvectorSolution t) {
		...
		for (int i = 0, size = this.getDimensions(); i < size; i++) {
			...
			index += this.bits[i];
			temp[i] = mins[i]
					+ ByteBuffer.wrap(variable).getInt() * ((maxs[i] - mins[i]) / (Math.pow(2, this.bits[i]) - 1));
		}
		return temp;
	}
	...
	public static byte[] toBytes(int val, int bufferSize) {
	...
	@Override
	public void setValues(BitvectorSolution t, double[] array) {
