 ...
 * 						third is where to calculate distance decision-space or objective-space
 * 						fourth is maximal number of iterations
 * @author Branko
 *
 */
public class MOOP {
	...
	public static void main(String[] args) {
		...
		if(maxiter<= 0 || sizeOfPopulation <= 0|| (fja!=1 && fja!=2) 
				|| (!space.matches("decision-space") && !space.matches("objective-space")) ){
			errorMsg();
		}
		...
		NSGA nsga = new NSGA(maxiter, maxs, mins, sizeOfPopulation, space, moop);
		...
		for(ArrayList<SolutionVector> front: pareto){
			for(SolutionVector vec : front){
				lines.add(vec.valuesToString());
				lines1.add(vec.solutionToString());
			}
			index++;
		}
	...
	private static void errorMsg() {
		...
		System.out.println("	third is where to calculate distance decision-space or objective-space");
		System.out.println("	fourth is maximal number of iterations");
		System.exit(0);
