 ...
 *
 */
public class Hypermutation implements IHypermutation {
	 ...
	 * @param rho probability of changing a value
	 * @param sigma change of variance
	 */
	public Hypermutation(double rho, double sigma) {
		this.rho = rho;
		this.sigma = sigma;
	...
	public ArrayList<Antigen<Double>> hypermutation(ArrayList<Antigen<Double>> antigens, double beta) {
		...
		for (int i = 0, size = antigens.size(); i < size; i++) {
			...
			double probability = Math.exp(-1 * rho * (fitness));
